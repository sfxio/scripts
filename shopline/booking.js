window.onload = function () {
    ...
    const producURL = decodeURIComponent(window.location.pathname).split('/')
    const productHandle = producURL[producURL.length - 1]
    let dataAll = {}
    let variant = {}
                ...
                const calendar = new HelloWeek({
                    ...
                    onSelect: () => {
                        ...
                        dataAll[selectedDate].forEach(element => {
                            ...
                            let mm = new Date(element.startTime).getMinutes() < 10 ? '0' + new Date(element.startTime).getMinutes() : +new Date(element.startTime).getMinutes()
                            let startTime = hh + ':' + mm
                            hh = new Date(element.endTime).getHours()
                            mm = new Date(element.endTime).getMinutes() < 10 ? '0' + new Date(element.endTime).getMinutes() : +new Date(element.endTime).getMinutes()
                            let endTime = hh + ':' + mm
    ...
    cart.addEventListener('click', function () {
        dataAll[selectedDate].forEach(element => {
            ...
            planIds = element.id + '_' + element.adminId + '_' + element.productId + '_' + element.variantId
        })
